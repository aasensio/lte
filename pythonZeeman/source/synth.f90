...
use mathsModule, only : saha, vecfvoigt, calculateDamping, planckFrequency, shortCharacteristics, computeHeight, gasPressure, vecfvoigt2, vecfvoigt_zeeman, strength_zeeman, formal_sol_polarized
...
! 				lineList%transition(i)%opacity(1,:,j) = lineList%transition(i)%lineOpacity * voigtProfile / (lineList%transition(i)%deltaNu * SQRTPI) !+ lineList%transition(i)%backOpacity					
